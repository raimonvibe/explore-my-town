...
export function SmartMap({ places, category }: SmartMapProps) {
  ...
  return (
    <div className="h-96 w-full rounded-lg overflow-hidden border-2 border-amber-300 shadow-lg">
      <MapContainer
        center={center}
        zoom={13}
          ...
          >
            <Popup className="vintage-popup">
              <div className="font-serif text-amber-900">
                ...
                <p className="text-sm text-amber-700 mb-2">{formatAddress(place)}</p>
                ...
                {Object.entries(place.tags)
                  ...
                  .slice(0, 2)
                  .map(([key, value]) => (
                    <span key={key} className="inline-block bg-amber-100 text-amber-800 text-xs px-2 py-1 rounded mr-1 mb-1">
                      {key}: {value}
                    </span>
                  ))}
              </div>
